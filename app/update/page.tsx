...
export default function UpdatePage() {
  ...
  function handleExecuteRemove() {
    setArchiveModelsText(archiveDraftText);
    setExecuteCount((current) => current + 1);
  }
        ...
        <div className={`update-content ${authReady && isUnlocked ? "" : "update-content--locked"}`}>
          <section className="update-hero">
            <article className="update-card">
              ...
              <h1>更新後台</h1>
              <p className="update-lead">
                這裡可以整理 Excel 機型、比對圖片、輸出更新摘要。現在已加入「執行移除」按鈕，
                先在下方輸入要移除的機型，再按按鈕才會套用。
              </p>
          ...
          <section className="update-grid">
            <article className="update-card">
              ...
              <div className="update-file-grid">
                ...
                <label className="update-file-box">
                  <span>圖片資料夾</span>
                  <strong>{imageFiles.length ? `${imageFiles.length} 個檔案` : "尚未選擇圖片"}</strong>
                  <input
                    accept="image/*"
                    multiple
            ...
            <article className="update-card">
              ...
              <div className="update-actions">
                <button className="button-primary" onClick={handleExecuteRemove} type="button">
                  執行移除
                </button>
                <span className="update-pill">已執行 {executeCount} 次</span>
              </div>
            ...
            <article className="update-card">
              ...
              {imageFiles.length === 0 ? (
                <div className="empty-state">
                  <strong>尚未選入圖片</strong>
                  <span>選入資料夾後，這裡會顯示可配對到的機型清單。</span>
                </div>
              ) : (
